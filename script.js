/* script.js - 完整整合版本，最大化效益最小化 LocationIQ 請求 */
...
/* ----- 快取 ----- */
const geoCache = new Map(); // key: address string, value: {lat, lon, raw, suggestions}
const overpassCache = new Map(); // key: `${lat},${lon},${radius},${type}`, value: results
...
/* ----- findRestaurants with filters, dedup, cache ----- */
async function findRestaurants(lat, lon, radius = 1000, type = ''){
  const cacheKey = `${lat},${lon},${radius},${type}`;
  ...
  overpassCache.set(cacheKey, filtered);
  return filtered;
}
...
function renderResults(restaurants){
  ...
  const ref = userLocation ? {lat:userLocation.lat, lon:userLocation.lon} : lastSearchCenter;
  ...
  top.forEach(item=>{
    ...
    const btnMaps = document.createElement("button"); btnMaps.textContent="在 Google Maps 開啟"; btnMaps.onclick=()=>{ window.open(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(name+' '+address)}`,"_blank"); };
...
streetInput.addEventListener('input',()=>{
  ...
  autocompleteTimer = setTimeout(async ()=>{
    ...
    try{
      ...
      streetSuggestions.innerHTML=''; suggestionItems=[]; selectedSuggestionIndex=-1;
    }
  },300);
});
...
searchBtn.addEventListener('click', async ()=>{
  ...
  let geo = null;
  if(geoCache.has(query) && geoCache.get(query).lat) geo=geoCache.get(query);
  else{
    const userTypedHasNumber = /\d/.test(street);
    geo = userTypedHasNumber ? await geocode(query) : null;
    if(!geo){
      try{
        const url=`https://nominatim.openstreetmap.org/search?format=json&addressdetails=1&countrycodes=TW&limit=1&q=${encodeURIComponent(city+' '+district+' '+street)}`;
        const r = await fetchWithTimeout(url,{ headers:{ "Accept":"application/json" }},8000);
        const j = await r.json();
        if(Array.isArray(j) && j.length>0) geo={ lat:parseFloat(j[0].lat), lon:parseFloat(j[0].lon), raw:j[0] };
      }catch(e){ console.warn("Nominatim fallback failed", e); }
    }
    if(!geo){ alert("找不到地點"); hideLoading(); setBusy(false); return; }
  ...
  let restaurants = [];
  try{ restaurants = await findRestaurants(geo.lat, geo.lon, radius, type); }
  catch(e){ console.error(e); alert("查詢餐廳失敗"); }
  renderResults(restaurants);
  hideLoading(); setBusy(false);
});

/* ----- reshuffle ----- */
reshuffleBtn.addEventListener('click',()=>{ if(lastRestaurants.length) renderResults(lastRestaurants); });
...
locateBtn.addEventListener('click',()=>{
  ...
  navigator.geolocation.getCurrentPosition(pos=>{
    ...
    userMarker=L.marker([userLocation.lat,userLocation.lon],{ title:"您的位置" }).addTo(map);
