...
async function overpassQuery(query){
  ...
  for(const endpoint of OVERPASS_SERVERS){
    try{
      ...
      if(!r.ok){ lastErr = new Error(`Overpass ${endpoint} status ${r.status}`); continue; }
      const txt = await r.text();
      if(typeof txt === 'string' && txt.trim().startsWith('<')){ lastErr = new Error('Overpass returned HTML error'); continue; }
...
async function findRestaurants(lat, lon, radius = 1000, type = ''){
  ...
  "hotpot":[
    ...
    (mapping[type] || mapping["restaurant"]).forEach(s=>filters.push(`${s}(around:${radius},${lat},${lon});`));
  }
  ...
  const filtered = elements.filter(e=>{
    const t = e.tags || {};
    if(t.disused || t.abandoned || t.closed || (t.opening_hours && /closed|off|休業|歇業|永久/i.test(t.opening_hours))) return false;
...
/* ----- Street autocomplete with Nominatim ----- */
...
streetInput.addEventListener('input', async ()=>{
  ...
  try{
    let url = `https://nominatim.openstreetmap.org/search?format=json&addressdetails=1&countrycodes=TW&limit=6&q=${encodeURIComponent(city+' '+district+' '+q)}`;
    const r = await fetchWithTimeout(url, { headers: { "Accept": "application/json" } });
    if(!r.ok) throw new Error('Nominatim suggestion error');
    const j = await r.json();
...
searchBtn.addEventListener('click', async ()=>{
  ...
  if(userTypedHasNumber && !geocodedHasHouseNumber){
    const ok = confirm("找不到精準門牌，是否改以街道/區域搜尋？");
    if(!ok){ hideLoading(); setBusy(false); return; }
  }
...
locateBtn.addEventListener('click', ()=>{
  ...
  navigator.geolocation.getCurrentPosition(pos=>{
    ...
    locateBtn.disabled = false; locateBtn.textContent = "以我位置搜尋";
  }, err=>{
    console.warn(err);
    alert("無法取得您的位置");
    locateBtn.disabled = false; locateBtn.textContent = "以我位置搜尋";
